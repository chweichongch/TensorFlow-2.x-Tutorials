...
class FasterRCNN(tf.keras.Model, RPNTestMixin, BBoxTestMixin):
    ...
    def call(self, inputs, training=True):
        ...
        用backbone函数得到C2，C3，C4，C5几个不同感受野的特征图
        
        将C2，C3，C4，C5输入到neck函数里得到FPN输出的P2, P3, P4, P5, P6几个不同的感受野的特征图
        
        rpn_feature_maps = [P2, P3, P4, P5, P6]为rpn的特征图集合
        rcnn_feature_maps = [P2, P3, P4, P5]为rcnn的特征图集合
        
        将rpn的特征图集合输入到rpn_head函数里得到rpn网络输出的穷举边框的分类效用rpn_class_logits，穷举边框的分类概率rpn_probs，
        穷举边框的位置rpn_deltas
        ...
        将抠出的特征子图集合pooled_regions_list输入到函数bbox_head中，
        得到rcnn_class_logits_list, rcnn_probs_list, rcnn_deltas_list 
        
        将rcnn_class_logits_list, rcnn_probs_list, rcnn_deltas_list输入到函数bbox_head.get_bboxes得到最终的预测边框detections_list
